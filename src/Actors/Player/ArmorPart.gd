...
func lerp_to_neutral_colors(delta) -> void:
	if transition_time <= 1:
		...
		for part in armor:
			material.set_shader_parameter("R_MainColor4", lerp(original_color1,NeutralColors[0], transition_time))
			material.set_shader_parameter("R_MainColor5", lerp(original_color2,NeutralColors[1], transition_time))
			material.set_shader_parameter("R_MainColor6", lerp(original_color3,NeutralColors[2], transition_time))
	else:
		going_to_neutral_colors = false
		transition_time = 0
...
func go_to_neutral_colors():
	...
	original_color1 = material.get_shader_parameter("R_MainColor4")
	original_color2 = material.get_shader_parameter("R_MainColor5")
	original_color3 = material.get_shader_parameter("R_MainColor6")
