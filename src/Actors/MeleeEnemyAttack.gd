...
func connect_combo(_previous_combo):
	_previous_combo.connect("damage_target", Callable(self, "_on_previous_combo_damage_target"))
...
func deal_damage(target) -> void:
	...
	if not dont_deal_damage:
		...
		if not GameManager.player.is_connected("damage", Callable(self, "on_actual_hit")):
			GameManager.player.connect("damage", Callable(self, "on_actual_hit"))
...
func on_actual_hit(_value, _inflicter = null):
	...
	GameManager.player.disconnect("damage", Callable(self, "on_actual_hit"))
