...
@onready var troia_particles: GPUParticles2D = $troia_particles
@onready var troia_particles_2: GPUParticles2D = $troia_particles2

func _ready() -> void:
	get_parent().connect("open_doors", Callable(self, "queue_free"))
