extends Sprite2D
@onready var manta_ray: CharacterBody2D = $"../../Scenery/CollapsingRepeating/Boss/MantaRay"

func _ready() -> void:
	manta_ray.connect("zero_health", Callable(self, "activate"))# warning-ignore:return_value_discarded
...
func activate() -> void:
	...
	tween.tween_callback(Callable(self, "play_stage_song"))
	tween.tween_callback(Callable(self, "teleport_player"))
